...
**Python 侧验收**:用 onnxruntime 加载后,两个输出都与 PyTorch 对齐,CLS cosine ≥ 0.999、patch 逐 token mean cosine ≥ 0.999(逐张对比 100 张)。对应 `Tools/verify_onnx_parity.py`。
...
- `Tools/cv_grid_design.ipynb` — A3.5 的 CV 网格设计 PoC notebook(用 numpy 跑全量 6 标量)
...
| `Tools/export_dinov3_onnx.py` | **M1 新增**:CLS + patch 双输出 wrapper 导出 | **A2-M1** |
| `Tools/verify_onnx_parity.py` | **M1 新增**:PyTorch vs ONNX 对齐校验(CLS + patch 两路) | **A2-M1** |
| `Tools/cv_grid_design.ipynb` | **A3 新增**:CV 网格设计 PoC notebook(numpy 跑全量 6 标量,评估 FFT 去留) | **A3** |
