...
- [Tools/export_dinov3_onnx.py](../Tools/export_dinov3_onnx.py) — 双输出 wrapper,`cls_embedding` + `patch_tokens`,用 `h[:, -num_patch_tokens:, :]` 切片自动跳过 register token,S/B/L 通用。
- [Tools/verify_onnx_parity.py](../Tools/verify_onnx_parity.py) — PyTorch vs ONNX 两路 cosine ≥ 0.999 校验。
...
| **T2. 全文件夹批量特征提取** | 新增批量提特征能力,让未查看过的照片也入库 | 相似聚类列表不再只看得到已查看过的照片 |
...
| **T5. 相似聚类面板增强** | 现有面板加时间差、分数直方图、调试开关 | 用户能对聚类结果作快速判断 |
...
**动作**:
1. 4080 桌面跑 [Tools/export_dinov3_onnx.py](../Tools/export_dinov3_onnx.py) → `PhotoViewer/Assets/Models/dinov3_vits16.onnx`。
2. 跑 [Tools/verify_onnx_parity.py](../Tools/verify_onnx_parity.py) 确认 cls_min / patch_min ≥ 0.999。
...
**解决**:进入文件夹后,后台批量给**所有照片**提 CLS 特征 + CV 网格 + patch token(三路一次前向),写入数据库;列表界面显示进度,完成前聚类列表用已入库照片的子集,完成后才是全量。

<!-- 产品设计待填区(由用户补充):
- 触发时机:自动(打开文件夹即跑) / 手动(按钮触发) / 可切换
- 进度呈现:文件栏顶部进度条 / 工具窗口单独页 / 状态栏文字
- 优先级策略:当前可见照片优先 / 按拍摄时间顺序 / 按文件名
- 可中断性:切换文件夹时终止旧任务 / 后台完成已开工的 / 用户可暂停
- 资源约束:同时几张?桌面 vs 移动端差异?电池/WiFi gate 是否二期做?
- 多模型策略:只跑当前 `ModelId`,还是允许预留 S+L 并存?
- 入库 schema:
  - CLS 继续写 `photos.feature_vector` / `feature_model` 单列(现状),还是二期直接 ALTER 到纵表?
  - patch 是否入库(默认否,等 T3 决策);若入库,是 f32 / f16 / INT8?
  - CV 网格是否入 `photos.cv_grid` 新列?
- 失败处理:单张失败跳过 / 累计失败率告警 / 无 EXIF 指纹的照片怎么办
- 已入库照片的刷新:ModelId 变更 / 算法变更如何标记失效
-->
...
| 文件 | 角色 |
|---|---|
| `PhotoViewer/Core/AI/FolderFeatureIndexer.cs` **新增** | 批量任务调度门面:`StartAsync(folder)` / `CancelAsync` / `ProgressChanged` 事件 |
| `PhotoViewer/Core/AI/CvGridCache.cs` **新增** | `photos.cv_grid` BLOB 读写(schema ALTER 随 T2 落地) |
| `PhotoViewer/Core/AI/PatchFeatureExtractor.cs` **新增(条件)** | 若产品决策"patch 要入库",消费 ONNX patch 端口 + avg pool 16×16 + 量化 |
| `PhotoViewer/Core/AI/PatchFeatureCache.cs` **新增(条件)** | patch 入库表的读写薄封装 |
| [Core/AI/DinoFeatureCache.cs](../PhotoViewer/Core/AI/DinoFeatureCache.cs) | 新增批量路径:接收 `IEnumerable<ImageFile>`,内部合并指纹去重 |
| [PhotoViewer/ViewModels/Main/File/](../PhotoViewer/ViewModels/Main/File/) | 新增进度 ViewModel,挂到文件栏某处(产品决策位置) |
| `PhotoViewer/Core/Database/PhotoDatabase.cs` | 按产品决策一次 ALTER:新列 / 新表 / 多模型纵表任选一档,保证**只动一次** |

**硬约束(不等产品决策)**:
- 所有写数据库的 SQL 必须带 `WHERE fingerprint = ? AND model_id = ?`;未来多模型并存时不会隐式混用。
- 批量任务必须可取消,文件夹切换时旧任务立即放弃。
- 入库失败单张跳过,不能因一张坏图阻塞整批。
...
在已有 [SimilarityListView](../PhotoViewer/Views/Main/File/SimilarityListView.axaml) 上加:

1. 每项分数旁附**拍摄时间差**(秒/分/小时),目测兄弟照片是否时间接近
2. 面板顶部**分数直方图缩略条**,展示当前文件夹所有候选的 cosine 分布(不仅 Top N),目测极相似照片是否聚在 ≥ 0.98 区间
3. 设置里加开关 `ShowSimilarityDebugInfo`(默认关),开启后显示原始 cosine 与 model_id

**不做**:t-SNE 二维散点图。列表 + 直方图已够回答 §0.3 场景 1/2/3/4 的聚类问题。
...
| # | 门槛 | 实现 | 何时跑 |
|---|---|---|---|
| 1 | ONNX vs PyTorch cosine ≥ 0.999 | [Tools/verify_onnx_parity.py](../Tools/verify_onnx_parity.py) | 每次导出 ONNX |
...
- [ ] 进入万张级文件夹后,进度条正常跑到 100%
- [ ] 中途切换文件夹,旧任务能在 1s 内取消,新任务即刻开工
- [ ] 完成后相似聚类列表显示的候选数 ≥ 文件夹总数的 95%(剩 5% 是无 EXIF 指纹等边界情况)
- [ ] 单张失败不阻塞整批
...
| 风险 | 等级 | 应对 |
|---|---|---|
| 全文件夹批量提取挤占 UI 线程,打开文件夹卡顿 | **高** | 调度/并发细则待产品决策后定;硬底线:批量任务必须可取消 |
| 万张级文件夹批量时间过长,用户等不及 | 中 | 进度呈现必须清晰;允许中途切走继续后台跑(产品决策) |
...
**M1 — 全文件夹批量特征提取**(§3.2):
1. 产品设计细则由用户填写(见 §3.2 空白区)
2. 按细则实现 `FolderFeatureIndexer` + 进度 UI + schema ALTER(如需)
3. 万张级文件夹实测
...
- [ ] **§3.2 全文件夹批量提取的产品设计**(见该节空白区):触发时机 / 进度呈现 / 优先级 / 中断性 / 资源约束 / 多模型策略 / 入库 schema / 失败处理 / 失效刷新
