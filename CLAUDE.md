...
- [Controls/](PhotoViewer/Controls/): `ThumbnailCard` (主缩略图列表与相似聚类列表共用的 90×138 卡片:缩略图 + 文件名 + 自定义第二行 + 6 星级 + 抖动徽标,徽标在 `File.IsShake == true` 时叠在 80×80 缩略图区右下角), `DetailPreview` (sidebar overview), `SortableList` (drag-reorder list for settings), `HotkeyButton` (hotkey capture), `CheckableMenuHeader`, `DeferredNumericTextBox`, `OverlayGlyphText`.
