...
public class ExifDetailViewModel : ReactiveObject
{
    ...
    public List<FormatEntry> Formats { get; }
    ...
    public ExifDetailViewModel(ImageFile imageFile)
    {
        ...
        // 默认选 RAW；无 RAW 时选第一个
        _selectedFormat = Formats.FirstOrDefault(f => RawExtensions.Contains(f.Label)) ?? Formats[0];
        _selectedFormat.IsSelected = true;

        // 若默认选中的格式未加载，立即触发加载
        if (_selectedFormat.Groups == null)
            _ = LoadFormatAsync(_selectedFormat);
    }

    private async Task LoadFormatAsync(FormatEntry entry)
    {
        ...
        try
        {
            ...
            {
                try { return await ExifLoader.LoadExifDataAsync(entry.File); }
                catch (Exception ex)
                {
