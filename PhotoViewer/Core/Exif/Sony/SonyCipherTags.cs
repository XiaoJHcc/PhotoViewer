...
internal static partial class SonyCipherTags
{
    ...
    /// <summary>
    /// 解码 Sony 0x940F 静照加速度计块（加密，ExifTool 尚无字段表）为显示字段。
    /// 数值真源在 <see cref="SonyAttitudeDecoder"/>（训练侧导出同用），此处只负责格式化。
    /// 始终导出三轴加速度（m/s²，按本帧 |raw| 归一到标准重力）；
    /// 俯仰/横滚仅对已校准机型（ILCE-7CM2 / ILCE-6700）计算。
    /// </summary>
    /// <param name="directory">Sony MakerNote 目录</param>
        ...
        SonyType1MakernoteDirectory directory, string? cameraModel = null)
    {
        ...
        if (!SonyAttitudeDecoder.TryDecode(directory, cameraModel, out var a))
            return null;
        ...
        if (a.PitchDeg is double pitchDeg && a.RollDeg is double rollDeg)
        {
            result.Add(new MetadataTag
            {
                TagId = tagId,
    ...
    internal static byte[] Decipher(byte[] encrypted)
    {
